...
**私聊与邮箱**：发词/投票说明优先好友私聊；失败时尝试带 `group_id` 的临时会话；**非好友须先私聊牛牛任意消息一次**后牛牛才能回信。仍失败且 `spy_email_fallback=true` 时，复用 **bot_status** 的 SMTP 向 `{QQ号}@qq.com` 发信。
...
| 现象 | 处理 |
| --- | --- |
| 收不到词语/投票私聊 | 非好友先私聊牛牛任意消息一次；或加好友；或查 QQ 邮箱；确认 bot_status SMTP 已配置 |
