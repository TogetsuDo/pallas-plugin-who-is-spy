...
| `私聊回复数字序号或 0` | 私聊 | 投票（0=弃权；私聊含本轮述词总结） |
...
**私聊与邮箱**：发词/投票说明优先好友私聊；失败时尝试带 `group_id` 的临时会话（NapCat 常不稳定）；SnowLuma 协议端**无法主动向非好友发私聊**，玩家须**先私聊牛牛任意消息一次**后牛牛才能回信。仍失败且 `spy_email_fallback=true` 时，复用 **bot_status** 的 SMTP 向 `{QQ号}@qq.com` 发信。
...
| 现象 | 处理 |
| --- | --- |
| 收不到词语/投票私聊 | 先私聊牛牛任意消息一次（SnowLuma 须）；或加好友；或查 QQ 邮箱；确认 bot_status SMTP 已配置 |
| 私聊「你的词：」后无字 | 非白板玩法，属投递失败；先私聊牛牛一次、加好友、查邮箱或看开局提示里的未达名单 |
